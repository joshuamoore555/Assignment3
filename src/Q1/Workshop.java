...
public class Workshop {
    private static int numberOfElves = 4;
    static Semaphore elvesOutside = new Semaphore(numberOfElves, true);
        ...
        public void run() {
            try {
                while (true) {
                    ...
                    elvesOutside.acquire(numberOfElves); //wait for elves to be outside
                    mutex.release(1);

                    System.out.println("Santa Entered "  + elvesInside.availablePermits()+ "  " + elvesOutside.availablePermits());
                    elvesOutside.release(numberOfElves); //allow elves to enter
